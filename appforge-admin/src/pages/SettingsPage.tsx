...
import { getSmtpConfig, updateSmtpConfig, testSmtp, getFcmConfig, updateFcmConfig, testFcm, changePassword } from '../lib/api';
import type { SmtpConfig, FcmConfig } from '../lib/api';
import { Mail, CheckCircle, XCircle, Loader2, Bell, Upload, Lock } from 'lucide-react';
      ...
      <div className="bg-white rounded-xl border border-gray-200/60 shadow-sm overflow-hidden">
        ...
        <form onSubmit={handleChangePassword} className="p-5 space-y-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-orange-500 to-red-600 text-white text-sm font-medium hover:from-orange-600 hover:to-red-700 transition-all disabled:opacity-50 inline-flex items-center space-x-2"
          >
            {pwdSaving && <Loader2 className="w-4 h-4 animate-spin" />}
            <span>{pwdSaving ? 'Guardando...' : 'Cambiar contraseña'}</span>
          </button>
        </form>
      </div>
    </div>
